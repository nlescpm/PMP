...
\begin{table}[!h]
\begin{booktabs}{colspec={|m{0.2\textwidth}|m{0.4\textwidth}|m{0.4\textwidth}|},row{even}={nlesc-yellow}}
    \toprule
    \rowfont{\bfseries}
    Stakeholder &  Responsibilities & More info \\[1.5ex]
    PM & 
    ...
    \end{itemize} 
      \end{minipage}
    & \\[1.5ex]
    Lead RSE &     \begin{minipage}[t]{0.4\textwidth}
    \begin{itemize}\itemsep0em
        \item monitors project hour expenditure and signals deviation from the workplan to the PM
    \end{itemize} 
      \end{minipage}
    &  Asks PM for hours status in Exact, or checks monthly budget status via Ganttic  \\[1.5ex]
    \bottomrule
\end{booktabs}
\end{table}
